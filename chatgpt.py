...
def call_deepseek_coder(messages, temp=0.8, model=config["model"]):
    print(f'[INFO] call deepseek coder, temp = {temp}, model = {model}')

    # Extract message content from the first message in messages list of dictionaries
    ...
    response = generator(message, max_length=2048, do_sample=True, top_p=0.95, top_k=100, temperature=temp)[0][
        'generated_text']
